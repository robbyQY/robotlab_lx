...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    arm_fixed_joint_pos = None
    arm_joint_ids = None
    if args_cli.arm_ee_pose is not None:
        arm_joint_names = ["joint1", "joint2", "joint3", "joint4", "joint5", "joint6"]
        arm_fixed_q = _solve_z1_ik(args_cli.arm_ee_pose)
        ...
        arm_fixed_joint_pos = torch.tensor(arm_fixed_q, dtype=torch.float32, device=sim_env.device).unsqueeze(0)
        arm_fixed_joint_pos = arm_fixed_joint_pos.repeat(sim_env.num_envs, 1)
        print(f"[INFO] Fixed arm EE pose (base_link): {args_cli.arm_ee_pose}")
        print(f"[INFO] IK solved arm joints {arm_joint_names}: {arm_fixed_q.tolist()}")
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            # agent stepping
            actions = policy(obs)
            # keep the arm at a fixed pose if requested
            if arm_fixed_joint_pos is not None:
                env.unwrapped.scene["robot"].set_joint_position_target(arm_fixed_joint_pos, joint_ids=arm_joint_ids)
